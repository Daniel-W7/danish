...
#define SSH     "/usr/bin/sftp"
#define SHELL   "/bin/bash"

//定义pg->type == PG_TYPE_SFTP的情况
void *wait_sftp_child(void *p)
{
    //pg_t为page.h中定义的结构体
    pg_t *pg = (pg_t*) p;
    //type为pt_g内部定义的枚举变量，包含PG_TYPE_HUB,PG_TYPE_SSH,PG_TYPE_SHELL
    if (pg->type == PG_TYPE_SFTP) {
...
void run_sftp(pg_t *pg)
{
    ...
    if (pg->ssh.child == 0) {
        ...
        execlp(SFTP, SFTP, host, "-p", pg->ssh.cfg.port, NULL);//执行系统命令，进行SFTP连接
    }
